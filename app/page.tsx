...
interface MenuItem { 
  ...
  description: string; 
  category: string;
  product_variants?: Variant[]; // Menampung data dari tabel product_variants
}
...
export default function DimsumWebsite() {
  ...
  // State Modal Pilihan Varian
  const [showVariantModal, setShowVariantModal] = useState(false)
  const [activeProduct, setActiveProduct] = useState<MenuItem | null>(null)
  const [selectedVariant, setSelectedVariant] = useState<Variant | null>(null)

  /* 1. FETCH DATA (BAGIAN YANG DIPERBAIKI) */
  useEffect(() => {
    const fetchData = async () => {
      // Menarik data menu sekaligus variannya menggunakan join query
  ...
  /* 3. LOGIKA POP-UP & KERANJANG */
  const handleOpenSelection = (item: MenuItem) => {
    setActiveProduct(item)
    // Jika ada varian, pilih varian pertama secara otomatis
  ...
  const confirmAddToCart = () => {
    ...
    // Buat kunci unik agar Sosis 15cm dan 25cm tidak tercampur
  ...
  const updateQuantity = (cartKey: string, delta: number) => {
    setCart((prev) => 
      prev.map((i) => i.cartKey === cartKey ? { ...i, quantity: i.quantity + delta } : i)
          .filter((i) => i.quantity > 0) // Otomatis hapus jika di-minus saat jumlah 1
    )
  }
  ...
  const handleCheckout = () => {
    ...
    if (checkedItems.length === 0 || !customerName.trim() || !address.trim()) {
      return alert("Lengkapi Nama, Alamat, dan pilih produk!")
    }
  ...
  return (
    <div className="min-h-screen bg-[#fdfaf5] text-[#3d2622] font-sans">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#3d2622]/10 p-4">
        <div className="container mx-auto flex justify-between items-center max-w-6xl">
          ...
          <Button variant="outline" className="relative border-[#d9a01e] text-[#d9a01e] rounded-2xl h-10 px-4" onClick={() => setIsCartOpen(true)}>
            ...
            {cart.length > 0 && <Badge className="absolute -top-2 -right-2 bg-[#d9a01e] text-white">{cart.filter(i => i.checked).length}</Badge>}
          </Button>
        </div>
      </header>
      ...
      {/* CATEGORY NAV */}
      <nav className="flex justify-center gap-3 mb-10 px-4">
        {["Makanan Asin", "Makanan Manis", "Frozen Food"].map((cat) => (
          <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-3 rounded-2xl font-bold text-xs transition-all ${selectedCategory === cat ? "bg-[#d9a01e] text-white shadow-lg scale-110" : "bg-white text-gray-400"}`}>{cat}</button>
        ))}
      </nav>
      ...
      <main className="container mx-auto px-4 pb-28 max-w-6xl">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
          {menu.filter(i => i.category.toLowerCase().trim() === selectedCategory.toLowerCase().trim()).map((item) => (
            <Card key={item.id} className="rounded-[2rem] overflow-hidden border-none shadow-md bg-white">
      ...
      {showVariantModal && activeProduct && (
        <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 animate-in slide-in-from-bottom duration-300">
            ...
            <p className="text-gray-500 font-bold text-sm mb-3">Pilih Varian Produk:</p>
            ...
            <Button className="w-full bg-[#d9a01e] h-14 rounded-2xl font-black text-white text-lg" onClick={confirmAddToCart}>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full p-6 flex flex-col shadow-2xl">
              ...
              {cart.map(item => (
                <div key={item.cartKey} className={`flex items-center gap-3 p-4 rounded-2xl border ${item.checked ? 'border-[#d9a01e] bg-[#fdfaf5]' : 'border-gray-100'}`}>
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-sm text-[#3d2622]">{item.name}</p>
                    {item.variantName && <p className="text-[10px] text-gray-400 font-bold mt-1">└ Varian: {item.variantName}</p>}
                  ...
                  <div className="flex items-center gap-2 bg-white border rounded-xl p-1">
                    <button onClick={() => updateQuantity(item.cartKey, -1)}><Minus className="w-3 h-3" /></button>
                    <span className="text-xs font-black w-4 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.cartKey, 1)}><Plus className="w-3 h-3" /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="pt-6 border-t mt-4 space-y-4">
              <div className="flex justify-between font-black text-lg text-[#3d2622]">
              ...
              <div className="relative">
                ...
                <Input placeholder="Alamat Lengkap Pengiriman" value={address} onChange={(e) => setAddress(e.target.value)} className="rounded-xl h-12 bg-gray-50 border-none pl-10" />
              </div>
              <Button className="w-full bg-[#d9a01e] h-14 rounded-2xl font-black text-white" onClick={handleCheckout}>
